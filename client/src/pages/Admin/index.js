...
export default function Admin() {
    ...
    const filterAppsByDate = (item) => {
        let result = (moment(item.date).isAfter(formState.startDate) || moment(item.date).isSame(formState.startDate)) && (moment(item.date).isBefore(formState.endDate) || moment(item.date).isSame(formState.endDate))
        return result;
    };
        ...
        return(
            <div className="container">
                <div className='row'>
                    ...
                    <div className='col-md-6'>
                        {apps.length > 0 ? (<AppList list={apps} />) : (<div></div>)}
                        {/* {forList ? (forList.map((p) => (
